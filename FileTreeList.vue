<template>
    ...
    <div>
        ...
        <Draggable v-model="data" :eachDroppable="() => false" :eachDraggable="() => false" class="mtl-tree" ref="tree" virtualization style="height: 500px"
            treeLine>
...
<!--                    <input class="mtl-checkbox mtl-mr" type="checkbox" v-model="stat.checked" />-->
                    <span v-if="!(stat.data.isEdit)" data-placeholder="enter name" class="mtl-ml"
                        @click="stat.open = !stat.open" @input="handleInput($event, node, stat)">{{
                            node.text }}</span>
                    <input v-else ref="inputRef" draggable="false" v-model="node.text" @keyup.enter="toggleEdit(stat)"
